...
def bbox_nms(flow_boxs, cur_boxes, flow_scores, cur_scores):
    '''
    flow_boxs: (N, 4)
    cur_boxes: (M, 4)
    flow_scores: (N)
    ...
    for i in tqdm(range(video_length-1)):
        ...
        try:
            ...
            if i == 0:
                ...
                # 本帧、上一帧 边框置信度NMS
                #  flow_boxs = bbox_nms(flow_boxs, boxs, flow_bbox_scores, scores)
                inputs, origin_img, center, scale = PreProcess(input_image, bboxs, scores, cfg)
        ...
        if previous_ids == 0:
            previous_ids = [i for i in range(len(preds))]
        ...
        if i>0:
            ...
            print(prev_ids, cur_ids)

            cur_len = len(box_b)
            cur_maps = -np.ones(shape=(cur_len,))
            for pos, num in enumerate(cur_ids):
                cur_maps[num] = previous_ids[prev_ids[pos]]

            prev_max = max(max(previous_ids), prev_max)
